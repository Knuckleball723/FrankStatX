...
fromstreamlit_gsheets import GSheetsConnection
import pandas as pd
...
# --- EMAIL CONFIG ---
EMAIL_SENDER = "knuckleballinc@gmail.com"EMAIL_RECEIVER = "knuckleballinc@gmail.com"
...
try:
    df = conn.read(spreadsheet=SHEET_URL, ttl=0)except Exception as e:
...
for i in range(12):
    ...
    # Get current values from the Google Sheet (or empty string if NaN)t_val = str(df.iloc[i, 0]) if i < len(df) and not pd.isna(df.iloc[i, 0]) else ""
    s1_val = str(df.iloc[i, 1]) if i < len(df) andnot pd.isna(df.iloc[i, 1]) else ""
    s2_val = str(df.iloc[i, 2]) if i < len(df) and not pd.isna(df.iloc[i, 2]) else ""
    s3_val= str(df.iloc[i, 3]) if i < len(df) and not pd.isna(df.iloc[i, 3]) else ""
...
# --- THE SCOUT LOGIC ---
def check_espn(protected_ids):try:
        ...
        for row in rows:
            ...
            for p_id in protected_ids:
                if p_id and p_id in row_text and len(p_id) > 2:
...
if st.button("💾 SAVE TO SHEET & SCAN ESPN"):
    ...
    # 2. Run the ESPN Scan
    flat_ids = [item for sublistin updated_data for item in sublist[1:] if item.strip() != ""]
    check_espn(flat_ids)
